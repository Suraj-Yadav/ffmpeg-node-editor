...
namespace Window {
	...
	std::array<FrameContext, NUM_FRAMES_IN_FLIGHT> g_frameContext;
	UINT g_frameIndex = 0;
	...
	std::array<ID3D12Resource*, NUM_BACK_BUFFERS> g_mainRenderTargetResource;
	std::array<D3D12_CPU_DESCRIPTOR_HANDLE, NUM_BACK_BUFFERS>
	...
	bool CreateDeviceD3D(HWND hWnd) {
		...
		if (D3D12CreateDevice(
				nullptr, featureLevel, IID_PPV_ARGS(&g_pd3dDevice)) != S_OK) {
			return false;
		}
...

		{
			...
			if (dxgiFactory->CreateSwapChainForHwnd(
					g_pd3dCommandQueue, hWnd, &sd, nullptr, nullptr,
					&swapChain1) != S_OK) {
				return false;
			}
			if (swapChain1->QueryInterface(IID_PPV_ARGS(&g_pSwapChain)) !=
				S_OK) {
				return false;
			}
			swapChain1->Release();
